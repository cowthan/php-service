...
class Queue {
	...
	public static function process($args) {
		try {
			...
			foreach($queues as $k => $storage_queue) {
				...
				switch($storage['api_type']) {
					...
					case 'openstack':
						foreach($files as $k => $v) { // No batch operation here
							if(!self::canKeepGoing()) { // Time same
								break;
							}
							try {
								switch($storage['api_type']) {
									...
									break;
									case 'openstack':
										$container = $StorageAPI->getContainer($storage['bucket']);
										try {
											$object = $container->getObject($v['key']);
				...
				// Put the DB::increment queues here
				...
				DB::update('queues', ['status' => 'failed'], ['attempts' => 5]);
			}
	...
	public static function logAttempt($ids) {
		try {
			if(is_array($ids)) {
				$db = DB::getInstance();
				$db->query('UPDATE ' . DB::getTable('queues') . ' SET queue_attempts = queue_attempts + 1 WHERE queue_id IN (' . implode(',', $ids) . ')');
				return $db->exec() ? $db->rowCount() : false;
			} else {
				return DB::increment('queues', ['attempts' => '+1'], ['id' => $ids]) ? 1 : false;
			}
		} catch(Exception $e) {
			throw new QueueException($e);
		}
	...
	public static function displayPixel() {
	...
	public static function canKeepGoing($safe_time=5) {
		if(self::$max_execution_time) {
			$executed_time = pow(10, -5) * (microtime(true) - G_APP_TIME_EXECUTION_START);
			return self::$max_execution_time - $executed_time > $safe_time;
		}
		return true;
	}
}
