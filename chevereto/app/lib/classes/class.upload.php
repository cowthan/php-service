...
class Upload {
	...
	public function exec() {
		...
		// Read Exif data
		if($this->extension == 'jpg' and function_exists('exif_read_data')) {
	...
	private function validateInput() {
		...
		if($this->type == 'file') {
			...
			*/
			...
			if(!G\is_image_url($this->source) and !G\is_url($this->source)) {
	...
	private function fixImageOrientation($image_filename, $exif) {
	...
	private function handleFlood() {
		...
		try {
			...
			$flood_db = $db->queryFetchSingle("
				...
				FROM ".DB::getTable('images')." WHERE image_uploader_ip='".G\get_client_ip()."' AND image_date_gmt >= DATE_SUB(NOW(), INTERVAL 1 MONTH)");
		...
		if($is_flood) {
			if(getSetting('flood_uploads_notify') and !$_SESSION['flood_uploads_notify'][$flood_by]) {
				try {
					...
					$message_report .= '<br>';
					$message_report .= '<b>Uploads per time period</b>'."<br>";
					...
					send_mail(getSetting('email_incoming_email'), 'Flood report IP '. G\get_client_ip(), $message_report);
	...
	private function isValidImageMime($mime) {
		return preg_match("@image/(gif|pjpeg|jpeg|png|x-png|bmp|x-ms-bmp|x-windows-bmp)$@", $mime);
	}
	
	private function isValidNamingOption($string) {
