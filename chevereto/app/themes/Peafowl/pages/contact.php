...
G\Handler::$vars['doctitle'] = _s('Contact') . ' - ' . G\Handler::$vars['doctitle'];
...
if($_POST) {
	...
	if(count($input_errors) > 0) {
		...
		$body		.= 'Name: '.$name."\n";
		$body		.= 'E-mail: '.$email."\n";
		$body		.= 'User: '.(CHV\Login::isLoggedUser() ? CHV\Login::getUser()['url'] : 'not user')."\n";
		$body		.= 'Subject: '.$_POST['subject']."\n\n";
		$body		.= 'Message: '.$message."\n\n";
		$body		.= 'IP: '.G\get_client_ip()."\n";
		$body		.= 'Browser: '.getenv("HTTP_USER_AGENT")."\n";
		$body		.= 'URL: '.G\get_base_url()."\n\n";
		...
		if($mail->Send()) {
			...
			$is_error = true;
			$error = _s('Mail error') . ': ' . $mail->DbgOut;
		}
...
<div class="content-width">
	<div class="c24 center-box">
		...
		<form method="post" class="form-content">
			...
			<p><?php echo $is_sent ? _s('We will be in touch soon.') : _s('If you want to send a message fill the form below.'); ?></p>
			...
			<div class="btn-container">
				<button class="btn btn-input default" type="submit"><?php _se('Send message'); ?></button> <span class="btn-alt"><?php _se('or'); ?> <a href="<?php echo G\get_base_url(); ?>"><?php _se('cancel'); ?></a></span>
			</div>
		</form>
	</div>
</div>
