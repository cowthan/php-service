...
$open_graph = [
	...
	'image'			=> CHV\get_system_image_url(CHV\getSetting('homepage_cover_image')),
...
foreach($open_graph  as $k => $v) {
	if(!$v) continue;
	echo '<meta property="og:'.$k.'" content="'.G\safe_html($v).'">' . "\n";
}
...
<body id="<?php echo (preg_match('/^\d/', G\get_route_name()) ? 'route-' : NULL) . G\get_route_name(); ?>" class="<?php echo $body_class; ?>">
...
<header id="top-bar" class="top-bar<?php if(in_array($body_class, ['landing', 'split']) and $top_bar_class == 'black') { echo ' transparent'; } ?><?php echo ' ' . $top_bar_class;?>">
    ...
    <div class="content-width">
		...
		?>
        <div id="logo" class="top-bar-logo<?php if(is_private_gate()) { ?> text-align-left<?php } ?>"><a href="<?php echo CHV\Login::isLoggedUser() ? (CHV\getSetting('logged_user_logo_link') == 'user_profile' ? CHV\Login::getUser()['url'] : G\get_base_url()) : G\get_base_url(); ?>"><img class="replace-svg" src="<?php echo CHV\get_system_image_url($logo_header); ?>" alt="<?php echo CHV\getSetting('website_name'); ?>"></a></div>
                    ...
                    <div class="pop-box-inner pop-box-menu<?php if($cols > 1) { ?> pop-box-menucols<?php } ?>">
						...
						<?php
							foreach($enabled_languages as $k => $v){
								echo '<li' . (CHV\get_language_used()['code'] == $k ? ' class="current"' : '') . '><a hreflang="'.str_replace('_', '-', $k).'" href="' . G\get_base_url('?lang=' . $k) . '">' . $v["name"] . '</a></li>'."\n";
								$count++;
							}
						?>
                        </ul>
