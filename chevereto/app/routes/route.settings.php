...
$route = function($handler) {
	try {
		...
		// Editable values
		$allowed_to_edit = ['name', 'username', 'email', 'avatar_filename', 'website', 'background_filename', 'timezone', 'language', 'status', 'is_admin', 'newsletter_subscribe', 'bio', 'show_nsfw_listings'];
		...
		if($_POST) {
			...
			switch($doing) {
				...
				case 'account':
					...
					$_POST['newsletter_subscribe'] = in_array($_POST['newsletter_subscribe'], ['On', 1]) ? 1 : 0;
					$_POST['show_nsfw_listings'] = in_array($_POST['show_nsfw_listings'], ['On', 1]) ? 1 : 0;	
