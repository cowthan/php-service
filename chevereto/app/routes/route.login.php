...
$route = function($handler) {
	try {
		...
		// Request log
		$request_log_insert = array('type' => 'login');
		$failed_access_requests = $handler::getVar('failed_access_requests');
		
		if(CHV\is_max_invalid_request($invalid_requests_day['day'])) {
		...
		// reCaptcha thing
		// Lets see if the user last attempt from this IP was good or not, in that case reCaptcha isn't needed
		/*if($captcha_needed) {
			$last_login_db = CHV\Requestlog::get(['type' => 'login', 'ip' => G\get_client_ip()], ['field' => 'date', 'order' => 'desc'], 1);
			if($last_login_db['request_result'] == 'success') {
		...
		if($_POST and !$is_error) {
			...
			if(!$is_error) {
				...
				if($is_login) {
					...
					$logged_user = CHV\Login::login($user_db['user_id'], 'password', $_POST['keep-login'] == 1);
