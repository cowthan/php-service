		...
		switch($doing) {
			...
			case 'edit': // EX 3X
				...
				$allowed_to_edit = [
					...
					'album'		=> ['name', 'privacy', 'album_id', 'description'],
				...
				if($editing['new_album']) {
					$new_album = ['new_album', 'album_name', 'album_privacy', 'album_description'];
				...
				switch($type) {
					...
					case 'image':
						...
						if($editing['new_album']) {
							$inserted_album = CHV\Album::insert($editing['album_name'], $source_image_db['image_user_id'], $editing['album_privacy'], $editing['album_description']);
						...
						$json_array['status_code'] = 200;
						$json_array['success'] = array('message' => 'Image edited', 'code' => 200);
					...
					case 'album':
						...
						if(isset($editing['album_id']) or $editing['new_album']) {
							$album_move = true;
							if($editing['new_album']) {
								$editing['album_id'] = CHV\Album::insert($editing['album_name'],  $source_album_db['album_user_id'], $editing['album_privacy'], $editing['album_description']);
							}
						...
						$json_array['status_code'] = 200;
						$json_array['success'] = array('message' => 'Album edited', 'code' => 200);
					...
					case 'category':
						...
						$json_array['status_code'] = 200;
						$json_array['success'] = array('message' => 'Category edited', 'code' => 200);
			...
			case 'add-user':
				...
				$json_array['status_code'] = 200;
				$json_array['success'] = array('message' => 'User added', 'code' => 200);
			...
			case 'add-category':
				...
				$json_array['status_code'] = 200;
				$json_array['success'] = array('message' => 'Category added', 'code' => 200);
				$json_array['category'] = $category;
			...
			case 'add-ip_ban':
				...
				$json_array['status_code'] = 200;
				$json_array['success'] = array('message' => 'IP ban added', 'code' => 200);
			...
			case 'add-storage':
				...
				$json_array['status_code'] = 200;
				$json_array['success'] = array('message' => 'Storage added', 'code' => 200);
				$json_array['storage'] = $storage;
			...
			case 'create-album':
				...
				$album_id = $album['new'] ? CHV\Album::insert($album['name'], $owner_id, $album['privacy'], $album['description']) : CHV\decodeID($album['id']);
			...
			case 'delete':
				...
				if($single) {
					...
					foreach($contents_db as $content_db) {
						...
						$contents_ids[] = $content_db[$db_field_prefix . '_id'];
					}
					...
					$delete = $Class_fn::deleteMultiple($ids);
			...
			case 'disconnect':
				...
				// Login connection is the only one for this user
				if(count($user['login']) == 1) {
					throw new Exception(sprintf('Add a password or another connection before deleting %s', $disconnect_label), 12);
				}
