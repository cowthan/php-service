...
<html>
	...
	<body>
		...
		<a href="{{$base}}login"><h1 id="title">AlphaGo第三代智能App管理后台</h1></a>
		...
		<footer class="text-center">
			Copyright &copy; - <a href="#">北京大洞网络科技有限公司</a>
		</footer>
		
		<!-- Javascript files -->
		<!-- jQuery -->
		<script src="{{$assets}}jquery/jquery.js"></script>
		<script src="{{$assets}}jquery/html5shiv.js"></script>
		<!-- Bootstrap JS -->
					...
					$.post('{{$base}}admin/login.json', {
						...
						"password" : signin_password.val()
						 ...
						 if(status === "success"){
							 if(res.code == 200){
								 layer.msg("成功了！");
								 window.location.href="{{$base}}admin/home&sid=" + res.result.sid;
							 }else{
					...
					$.post('{{$base}}admin/regist.json', {
						...
						if(status === "success"){
							...
							if(res.code == 200){
								...
								layer.msg("注册失败：" + res.result);
							}
						 }else{
							 layer.alert("注册失败--" + status);
						 }
					 });
					return false;
				});
